...
RollingHash<5000, int, 1000000007, 'a', 0> hsh;
...
int main() {
	...
	hsh.init(S.begin(), S.end(), 131);
	for (int i = 1, len = S.size(); i <= len >> 1; i++) {
		memset(vis, false, sizeof(vis));
		for (int j = 0; j <= len - i; j++) {
			if (vis[j]) continue;
			int cur = hsh.get_hash(j, j + i - 1);
			for (int k = j + i; k <= len - i && cur == hsh.get_hash(k, k + i - 1); k += i)
				ans.insert(hsh.get_hash(j, k + i - 1)), vis[k] = true;
		}
	}

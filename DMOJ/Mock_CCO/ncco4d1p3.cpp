...
int main() {
	...
	while (M--) {
		cin >> A >> B >> W;
		sp1.add_bi(A, B, W); sp2.add_bi(A, B, W);
		edges.push_back({{A, B}, W}); edges.push_back({{B, A}, W});
	}

	sp1.min_path(1, N); sp2.min_path(N, N);
	int sp = sp1.dist[N], ans = INT_MAX;
	for (auto e : edges) {
		int cur = sp1.dist[e.first.first] + e.second + sp2.dist[e.first.second];
		if (cur > sp) ans = min(ans, cur);
	}
	cout << ans << '\n';
	return 0;
}

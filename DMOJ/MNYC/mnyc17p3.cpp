...
RollingHash<1000000, int, 1000000007, 'A', 0> ahash, bhash;
...
int main() {
	...
	ahash.init(A.begin(), A.end()); bhash.init(B.begin(), B.end());
	int pos = 0;

	for (int i = 1; i <= int(min(A.size(), B.size())); i++) {
		if (ahash.get_hash(A.size() - i, A.size() - 1) == bhash.get_hash(0, i - 1)) {
			pos = i;
		}
	}
	cout << A + B.substr(pos, B.size()) << '\n';
}
